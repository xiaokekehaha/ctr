...
def fun(user_feature, spid_feature):
	if ' 3:' in user_feature:
		return False
	if ' 69:' in spid_feature:
		return False
	return True
...
user_ctr_dict = {}
user_feature = {}
for line in open('feature_user_' + month):
	line = line.strip()
	idx = line.index(' ')
	user = line[2 : idx]
...
spid_ctr_dict = {}
spid_feature = {}
for line in open('feature_spid_' + month):
	line = line.strip()
	idx = line.index(' ')
	spid = line[2 : idx]
...
caid_feature = {}
for line in open('feature_caid_' + month):
	line = line.strip()
	idx = line.index(' ')
	caid = line[2 : idx]
...
browser_feature = {}
for line in open('feature_browser_' + month):
	line = line.strip()
	idx = line.index(' ')
	user = line[2 : idx]
...
for line in sys.stdin:
	...
	if last_user != cur_user and last_user != '':
		...
		for ans_str in ans_list:
			...
			if label == '0':#filter
				if fun(user_feature[last_user], spid_feature[spid]):
					continue
				else:
